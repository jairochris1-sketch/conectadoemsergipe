  ...
  return (
    <div className="fb-box">
            ...
            {pendingRequests.map((req) => (
              <div key={req.id} className="border border-border p-1.5 bg-accent">
                ...
                <div className="flex gap-1.5 mt-1.5">
                  <button onClick={() => acceptFriendRequest(req.id)} className="bg-primary text-primary-foreground border-none px-2 py-[3px] text-[10px] cursor-pointer hover:opacity-90">
                    {t("friends.accept")}
                  </button>
                  <button onClick={() => rejectFriendRequest(req.id)} className="bg-muted text-foreground border border-border px-2 py-[3px] text-[10px] cursor-pointer hover:opacity-90">
                    {t("friends.reject")}
                  </button>
                </div>
              </div>
      ...
      <div className="p-2 space-y-1">
        {friends.length > 0 ? (
          friends.map((friend) => (
            <Link key={friend.id} to={`/user/${friend.id}`} className="flex items-center gap-1.5 text-[11px] no-underline hover:bg-accent p-1">
              ...
              <div className="flex items-center gap-0.5">
                <span className="font-bold hover:underline text-[11px]">{friend.name}</span>
                <VerificationBadge {...(badges.get(friend.id) || {})} />
              </div>
            </Link>
          ))
        ) : (
          <p className="text-[11px] text-muted-foreground p-1">{t("friends.none")}</p>
        )}
      </div>
      {friends.length > 0 && (
        <div className="border-t border-border px-2 py-1.5">
          <Link to="/amigos" className="text-[11px] hover:underline">{t("see_all_friends")}</Link>
        </div>
      )}
    </div>
  );
