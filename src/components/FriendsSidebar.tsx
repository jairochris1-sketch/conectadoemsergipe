...
const FriendsSidebar = () => {
  ...
  return (
    <div className="bg-card border border-border p-3 w-full">
        ...
        <div className="mb-3">
          ...
          <div className="space-y-2">
            {pendingRequests.map((req) => (
              <div key={req.id} className="border border-border p-2 bg-accent">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => acceptFriendRequest(req.id)} className="bg-primary text-primary-foreground border-none px-3 py-1.5 text-sm cursor-pointer hover:opacity-90 rounded-sm">
                    {t("friends.accept")}
                  </button>
                  <button onClick={() => rejectFriendRequest(req.id)} className="bg-muted text-foreground border border-border px-3 py-1.5 text-sm cursor-pointer hover:opacity-90 rounded-sm">
                    {t("friends.reject")}
                  </button>
                </div>
              </div>
      ...
      <div className="border-b border-border pb-2 mb-2">
        <h3 className="text-base font-bold text-primary">{t("friends")} ({friends.length})</h3>
      </div>
      <div className="space-y-2">
        {friends.length > 0 ? (
          friends.map((friend) => (
            <Link key={friend.id} to={`/user/${friend.id}`} className="flex items-center gap-2 text-sm no-underline text-primary hover:bg-accent p-1.5 rounded">
              <div className="relative w-[36px] h-[36px] bg-muted border border-border flex items-center justify-center overflow-hidden shrink-0 rounded-sm">
                {friend.photo ? (
                ...
                {onlineUsers.has(friend.id) && (
                  <span className="absolute -bottom-[1px] -right-[1px] w-[9px] h-[9px] rounded-full bg-green-500 border border-card" style={{ boxShadow: "0 0 3px rgba(34,197,94,0.6)" }} />
                )}
              </div>
              <div>
      ...
      {friends.length > 0 && (
        <div className="mt-3 border-t border-border pt-2">
          <Link to="/amigos" className="text-sm text-primary hover:underline">{t("see_all_friends")}</Link>
        </div>
      )}
    </div>
  );
